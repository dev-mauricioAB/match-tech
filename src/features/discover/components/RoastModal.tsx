...
export function RoastModal({
  ...
  const roastText = getDisplayedRoast(profile, activePersonaView);
      ...
      <div className="flex min-h-full items-start justify-center p-4 pt-24 pb-20 sm:p-8 sm:pt-28">
        ...
        >
          <div className="absolute -top-6 -right-4 sm:-right-6 z-10">
            <button
              onClick={onClose}
              className="bg-neo-pink text-white border-4 border-neo-black w-14 h-14 sm:w-16 sm:h-16 rounded-full font-black text-xl sm:text-2xl shadow-[4px_4px_0_0_#000] hover:translate-y-1 hover:translate-x-1 hover:shadow-none transition-all flex items-center justify-center cursor-pointer"
            >
              X
            </button>
          </div>
          ...
          <div className="bg-white border-[6px] border-neo-black shadow-[12px_12px_0_0_#B8FF29] sm:shadow-[16px_16px_0_0_#B8FF29] flex flex-col">
            <div className="bg-neo-black p-6 sm:p-8 flex items-center gap-4 text-neo-lime relative overflow-hidden">
              <div className="absolute inset-0 opacity-20 bg-[radial-gradient(#B8FF29_1px,transparent_1px)] bg-size-[12px_12px]" />
              ...
              <div className="mt-8 pt-6 border-t-4 border-black border-dashed flex flex-col md:flex-row gap-4">
                <button
                  className={`flex-1 py-4 px-3 border-[3px] border-black bg-neo-black hover:bg-white text-neo-pink hover:text-black text-xs md:text-sm font-black uppercase tracking-widest transition-all cursor-pointer ${
                    isBrutalActive
                      ? "opacity-50 cursor-not-allowed border-dashed shadow-none translate-y-0"
                  ...
                  onClick={async () => {
                    ...
                    await onGenerateRoast(profile, "brutal");
                  }}
                >
                  {profile.roastBrutal ? "VER TECH LEAD (BRUTAL)" : "GERAR TECH LEAD (BRUTAL)"}
                </button>
                ...
                <button
                  ...
                  onClick={async () => {
                    ...
                    await onGenerateRoast(profile, "mild");
                  }}
                >
                  {profile.roastMild ? "VER MENTOR (SUAVE)" : "GERAR MENTOR (SUAVE)"}
                </button>
              </div>
            </div>
